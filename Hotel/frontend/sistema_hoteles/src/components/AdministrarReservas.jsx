...
const AdministrarReservas = () => {
  ...
  const fetchReservations = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8092/reservas/detalle/todas`);
      if (response.ok) {
  ...
  const cancelarReserva = async (idReserva) => {
    try {
      const response = await fetch(`http://localhost:8092/reservas/${idReserva}/cancelar`, {
        method: 'PUT',
        headers: {
  ...
  const actualizarReserva = async (reservaActualizada) => {
    ...
    try {
      const response = await fetch(`http://localhost:8092/reservas/${reservaActualizada.idReserva}`, {
        method: 'PUT',
  ...
  const fetchUserReservations = async (userId) => {
    try {
      const response = await fetch(`http://localhost:8092/reservas/historial/${userId}`);
      if (!response.ok) {
