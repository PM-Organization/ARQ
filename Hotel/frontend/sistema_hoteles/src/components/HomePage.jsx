...
// FIN SEGUNDO FILTRO
  ...
  const fetchCapacidades = async () => {
    try {
      const response = await fetch('http://localhost:8091/habitaciones/capacidades');
  ...
  const fetchHotelImages = async (idHotel) => {
    try {
      const response = await fetch(`http://localhost:8091/hoteles/${idHotel}/imagenes`);
  ...
  const fetchRoomTypes = async () => {
    try {
      const response = await fetch('http://localhost:8091/tipos_habitacion');
  ...
  const fetchPaises = async () => {
    try {
      const response = await fetch('http://localhost:8091/hoteles/pais');
  ...
  const fetchInitialHotelsAndRooms = async () => {
    try {
      const hotelsResponse = await fetch('http://localhost:8091/hoteles');
      if (!hotelsResponse.ok) throw new Error('Error al cargar hoteles');
      const hotelsData = await hotelsResponse.json();
  
      const roomTypesResponse = await fetch('http://localhost:8091/tipos_habitacion');
      ...
      const hotelsWithRooms = await Promise.all(hotelsData.map(async (hotel) => {
        const roomsResponse = await fetch(`http://localhost:8091/habitaciones?hotelId=${hotel.id_hotel}`); 
        if (!roomsResponse.ok) {
  ...
  const fetchHotelsAndRoomsFiltered = async () => {
    ...
    try {
        const response = await fetch(`http://localhost:8091/habitaciones/buscar${queryParams}`);
