...
const AddHabitacionPage = () => {
  ...
  const fetchHabitaciones = async (idHotel) => {
    try {
      const url = idHotel ? `http://localhost:8092/habitaciones?hotelId=${idHotel}` : 'http://localhost:8092/habitaciones';
  ...
  const handleSubmitHabitacion = async (e) => {
    ...
    try {
        ...
        const response = await fetch('http://localhost:8092/habitaciones', {
            method: 'POST',
...
const iniciarEdicion = (habitacion) => {
  ...
  const guardarEdicion = async (idHabitacion) => {
    try {
      const response = await fetch(`http://localhost:8092/habitaciones/${idHabitacion}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
const cambiarEstadoHabitacion = async (idHabitacion, estadoActual) => {
  ...
  try {
    const response = await fetch(`http://localhost:8092/habitaciones/${idHabitacion}/estado/${nuevoEstado}`, {
      method: 'PUT',
    });
    if (!response.ok) {
