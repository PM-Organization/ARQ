...
const CheckoutPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const responseDisponibilidad = await fetch('http://localhost:8092/reservas/verificar-disponibilidad', {
      method: 'POST',
    ...
    try {
      ...
      const response = await fetch('http://localhost:8092/reservas', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    const verificarDisponibilidad = async () => {
      if (reservationData.checkIn && reservationData.checkOut) {
        ...
        try {
          const response = await fetch(`http://localhost:8092/verificar-disponibilidad`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              idHabitacion: roomDetails.idHabitacion,
