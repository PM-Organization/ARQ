...
const AddHotelPage = () => {
  ...
  const fetchHoteles = async () => {
    try {
      const response = await fetch('http://localhost:8092/hoteles');
  ...
  const guardarEdicion = async () => {
    try {
      const response = await fetch(`http://localhost:8092/hoteles/${editandoId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
const manejarImagenesDelHotel = async (hotelId, imageLinks) => {
  ...
  try {
    const response = await fetch(`http://localhost:8092/hoteles/${hotelId}/imagenes`, {
  ...
  const eliminarHotel = async (id) => {
    try {
      const response = await fetch(`http://localhost:8092/hoteles/${id}`, {
        method: 'DELETE',
      });
...
const fetchHotelImages = async () => {
  ...
  for (let hotel of hoteles) {
    const response = await fetch(`http://localhost:8092/hoteles/${hotel.id_hotel}/imagenes`);
    if (response.ok) {
...
const cambiarEstadoHotel = async (idHotel, estadoActual) => {
  ...
  try {
    const response = await fetch(`http://localhost:8092/hoteles/${idHotel}/estado/${nuevoEstado}`, {
      method: 'PUT',
    });
    if (!response.ok) {
